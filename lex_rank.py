...
class LexRankSummarizer(AbstractSummarizer):
    ...
    def __call__(self, document, sentences_count, p_vector, matrix):
        ...
        scores = self.power_method(matrix, self.epsilon, p_vector)
        ratings = dict(zip(document.sentences, scores))

        return self._get_best_sentences(document.sentences, sentences_count, ratings), ratings
    ...
    @staticmethod
    def power_method(matrix, epsilon, p_vector):
